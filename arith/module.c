...
int LincrBy(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
 ...
 // we must have at least 4 args
  ...
  RMUTIL_ASSERT_NOERROR(rep);
  //RedisModuleString *temp = RedisModule_CreateStringFromCallReply(rep);
  //long long temp_val;
  //RedisModule_StringToLongLong(temp,&temp_val);
  //RedisModule_ReplyWithLongLong(ctx,temp_val);
  long long listLength;
  listLength = RedisModule_CallReplyInteger(rep);
  //RedisModule_ReplyWithLongLong(ctx,listLength);
  ...
  if(startIndex > endIndex && endIndex != -1) {
  	RedisModule_ReplyWithError(ctx,"ENDINDEX SHOULD BE LESS THAN OR EQUAL TO ENDINDEX");
  }
